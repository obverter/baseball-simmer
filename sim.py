...
def explain_to_file(sample_best_seasons, runs_matrix, name_to_index, num_seasons, output_file):
    with open(output_file, 'w') as fp:
        ...
        index_to_name = {v: k for k, v in name_to_index.iteritems()}
...
def rbi_ordered_seasons_expl(lineup, ordered_seasons, name):
    ...
    for p in lineup_copy:
        if p.get_name() == name and this_player is None:
            this_player = p
            break
    ...
    for lis in ordered_seasons:
        ...
        count = 0.0
        for s in lis:
            this_player = next((p for p in s.get_lineup() if p.get_name() == name), None)
